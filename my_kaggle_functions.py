...
def plot_features_eda(df: pd.DataFrame, features: List[str], target: str, 
    ...
    def _plot_cat_tgt_cat_relationship(ax, feature, order):
        try:
            if (df[feature].cat.categories.dtype == 'int' or 
                df[feature].cat.categories.dtype == 'float'):
                df_plot[feature] = df_plot[feature].astype('str').astype('category')
    ...
    for i, feature in enumerate(plot_features):
        ...
        if is_num:
            ...
            if tgt_cat: 
                ...
                _plot_num_relationship(fig.add_subplot(gs[i, 1]), feature, log_scale=scale_log)
            continue
        ...
        if is_cat:
            ...
            if len(order) <= 32:
                ...
                if df_plot[feature].dtype == 'category':
...
def calculate_score(actual, predicted, metric='rmse')-> float:
    ...
    if 'rmse_log' in metric or 'rmsle' in metric or 'log_rmse' in metric:
        ...
        return skl.metrics.balanced_accuracy_score(actual, predicted)
...
def plot_training_results(X_t: pd.DataFrame, X_v: pd.DataFrame,
    ...
    def _plot_distribution(ax, hist=True):
        if hist == True:
            ax.hist(y_v.astype(float).ravel(), color='xkcd:silver', alpha=0.8, density = True)
            ax.hist(y_p.astype(float).ravel(), color='xkcd:ocean blue', alpha=0.9, density = True)
        else: 
    ...
    if embed_v is not None:
        ...
        pca_proj = pca_projection.fit_transform(embed_v_sample)
        residuals = y_p - y_v
